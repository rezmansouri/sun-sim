...
def get_sim(sim_path):
    v_bc_path, v_path = [path_join(sim_path, file_name) for file_name in FILE_NAMES]
...
class Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        sims, self.v_min, self.v_max = min_max_normalize(
            sims, v_min, v_max
        )
        self.k, _, self.i, self.j = sims.shape
        self.sims = sims
