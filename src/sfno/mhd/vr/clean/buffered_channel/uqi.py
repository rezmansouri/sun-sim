...
def main():
    ...
    with torch.no_grad():
        for cube, paths in tqdm(val_loader, leave=False):
            ...
            mses_batch = uqi_per_sample(cube, yhats)
    ...
    with open(
        os.path.join(result_path, "evaluation_uqi.json"), "w", encoding="utf-8"
    ) as f:
        json.dump(
            {
                "uqi": float(mse),
    ...
    df = pd.DataFrame({"uqi": mses, "path": final_paths})
    df.to_csv(os.path.join(result_path, "evaluation_uqi.csv"), index=False)
