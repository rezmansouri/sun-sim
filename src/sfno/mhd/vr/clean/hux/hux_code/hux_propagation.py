...
def apply_hux_f_model(r_initial, dr_vec, dp_vec, r0=30 * 695700, alpha=0.15, rh=50 * 695700, add_v_acc=True,
    ...
    """
    # print(type(r_initial), type(r_initial[0]))
    v = np.zeros((len(dr_vec) + 1, len(dp_vec) + 1))  # initialize array vr.
    # print("v shape: ", np.shape(v), "r_initial shape: ", np.shape(r_initial))
    v[0, :] = r_initial
    ...
    for i in range(len(dr_vec)):
        for j in range(len(dp_vec) + 1):
            ...
            if j == len(dp_vec):  # force periodicity
                ...
                # print("omega_rot:", omega_rot, "type:", type(omega_rot))
                # print("dr_vec[i]:", dr_vec[i], "type:", type(dr_vec[i]))
                # print("frac2:", frac2, "unit:", getattr(frac2, 'unit', 'no unit'))

                # print("frac1: ", frac1, "frac2: ", frac2, 'v[i, j]: ', v[i, j])
