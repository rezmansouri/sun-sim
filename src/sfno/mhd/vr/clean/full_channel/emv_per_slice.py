...
def main():
    ...
    with torch.no_grad():
        for instance in tqdm(val_loader, leave=False):
            ...
            emvs_batch = emv_per_slice(cube, yhats)
    ...
    with open(
        os.path.join(result_path, "evaluation_emv_per_slice.json"),
        "w",
        encoding="utf-8",
    ) as f:
        json.dump(
    ...
    df.to_csv(os.path.join(result_path, "evaluation_emv_per_slice.csv"), index=False)
