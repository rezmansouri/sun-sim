...
def train(
    ...
    for epoch in range(n_epochs):
        ...
        for batch in tqdm(
            ...
            brx = batch["brx"].to(device)
            btx = batch["btx"].to(device)
            bpx = batch["bpx"].to(device)
            jrx = batch["jrx"].to(device)
            jtx = batch["jtx"].to(device)
            jpx = batch["jpx"].to(device)

            bry = batch["bry"].to(device)
            bty = batch["bty"].to(device)
            bpy = batch["bpy"].to(device)
            jry = batch["jry"].to(device)
            jty = batch["jty"].to(device)
            jpy = batch["jpy"].to(device)

            optimizer.zero_grad()
            brpred, bt_pred, bp_pred, jr_pred, jt_pred, jp_pred = model(
                [brx, btx, bpx, jrx, jtx, jpx]
            )
            data_loss = (
                loss_fn(brpred, bry)
                + loss_fn(bt_pred, bty)
            ...
            if physics_informed:
                physics_loss_value = physics_loss(
                    brpred, bt_pred, bp_pred, jr_pred, jt_pred, jp_pred
                )
                running_physics_loss += physics_loss_value.item() * brx.size(0)
            ...
            running_loss += loss.item() * brx.size(0)
