...
class Trainer:
    ...
    def train_one_batch_with_law(self, idx, sample, training_loss):
        """Run one batch of input through model and return training loss on outputs.
        Incorporates a second loss term calculated using the law function, where
        law outputs a tensor with the same shape as the model's predictions.
        ...
        sample : dict
            data dictionary holding one batch
        law : function
            a function that calculates an additional loss term based on model prediction and p
        ...
        # Calculate the law output (same shape as out)
        p = sample["p"]  # Extract p from sample
        ...
        if self.mixed_precision:
            with torch.autocast(device_type=self.autocast_device_type):
                loss += training_loss(out, **sample) + law_loss
        else:
            loss += training_loss(out, **sample) + law_loss
